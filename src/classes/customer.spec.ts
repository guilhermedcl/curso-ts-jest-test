...
describe('IndividualCustomer', () => {
  it('should have firstName, lastName and cpf', () => {
    const sut = createIndividualCustomer('Guilherme', 'Larroide', '111.111');
    expect(sut).toHaveProperty('firstName', 'Guilherme');
    expect(sut).toHaveProperty('lastName', 'Larroide');
    expect(sut).toHaveProperty('cpf', '111.111');
  });

  it('should have methods to get name and idn for individual customers', () => {
    const sut = createIndividualCustomer('Guilherme', 'Larroide', '111.111');
    expect(sut.getName()).toBe('Guilherme Larroide');
